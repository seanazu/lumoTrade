...
from src.llm.analyst import market_analyst
...
class HybridPredictionEngine:
    ...
    async def generate_prediction(
        ...
        """
        Generate comprehensive multi-horizon prediction with robust error handling
        ...
        def log_error(step_name: str, error: str, continue_anyway: bool = True):
            """Log errors but allow continuation"""
            if debug:
                debug_info["errors"].append({
                    "timestamp": datetime.now().isoformat(),
                    "step": step_name,
                    "error": error,
                    "continued": continue_anyway
                })
            print(f"⚠️  Error in {step_name}: {error}")
            if continue_anyway:
        ...
        try:
            current_prices = await asyncio.wait_for(
                self._fetch_current_prices(symbol),
                timeout=30  # 30 second timeout for data fetch
            )
            if debug:
            ...
            log_error("Fetch Market Data", "Timeout after 30s, using fallback data", True)
            current_prices = {"symbol": symbol, "price": 0, "volume": 0, "change": 0, "change_percent": 0}
            if debug:
                debug_info["stages"].append({
                    "name": "Fetch Market Data",
                    "duration_ms": 30000,
            ...
            log_error("Fetch Market Data", str(e), True)
            current_prices = {"symbol": symbol, "price": 0, "volume": 0, "change": 0, "change_percent": 0}
            if debug:
                debug_info["stages"].append({
                    "name": "Fetch Market Data",
        ...
        # Save to InstantDB
        try:
            from src.database.instantdb import instantdb_client
            for horizon, pred in final_predictions.items():
                prediction_id = f"pred_{symbol}_{horizon}_{current_time.strftime('%Y%m%d_%H%M%S')}"
                instantdb_client.store_prediction(
            ...
            print(f"⚠️  Failed to save prediction to InstantDB: {e}")
    ...
    async def _get_llm_adjustments(
        ...
        # If no base predictions (LightGBM not loaded), use all horizons for LLM-only mode
        horizons_to_predict = list(base_predictions.keys()) if base_predictions else self.horizons
        ...
        return {
            ...
            "features": payload["features"]  # Pass features for LLM-only fallback
        }
    ...
    def _fuse_predictions(
        ...
        """Fuse base ML and LLM predictions, or generate LLM-only predictions if base_ml is empty"""
        fused = {}
        
        # If no LightGBM models, generate simple LLM-only predictions
        if not base_ml:
            print("⚠️  No LightGBM models available - generating LLM-only predictions")
            for horizon in self.horizons:
                # Generate simple predictions based on sentiment
                # Positive sentiment = small positive move, negative = small negative move
                sentiment = llm_adj.get("features", {}).get("news", {}).get("sentiment_weighted_mean", 0)
                
                fused[horizon] = {
                    "mean": sentiment * 0.5,  # Scale sentiment to percentage move
                    "p10": sentiment * 0.5 - 1.0,
                    "p90": sentiment * 0.5 + 1.0,
                    "direction": "bullish" if sentiment > 0 else "bearish" if sentiment < 0 else "neutral",
                    "confidence": 0.5,  # Lower confidence without ML models
                    "source": "llm_only",
                    "model_details": {
                        "lightgbm": False,
                        "llm": True,
        ...
        # Normal fusion with LightGBM predictions
        for horizon in self.horizons:
            ...
            # Determine source based on LLM adjustments
            # Check if LLM provided meaningful adjustments
            has_llm_adjustment = (
                llm_adj and 
                isinstance(llm_adj, dict) and 
                (llm_adj.get("reasoning") or llm_adj.get("qualitative_risks"))
            )
            ...
            fused[horizon] = {
                ...
                "source": "hybrid" if has_llm_adjustment else "lightgbm",
                "model_details": {
                    "lightgbm": True,
                    "llm": has_llm_adjustment,
                    "fusion_weights": {
                        "ml_weight": self.base_ml_weight,
                        "llm_weight": self.llm_weight if has_llm_adjustment else 0
                    }
                }
            }
